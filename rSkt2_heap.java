...
public class GeneralCountMin {
	...
	public static PriorityQueue<Long> fqueue;
	public static int ssnum=200;
	public static int[] finalresult=new int[200];
	public static HashMap<Long,Integer> smap=new HashMap<Long,Integer>();
	public static HashSet<Long> ssset=new HashSet<Long>();
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		for (int i : spreadMeasurementConfig) {
			for(int ttime=0;ttime<GeneralUtil.timeround;ttime++) {
				times=ttime;
				initCM(i);
				encodeSpread(GeneralUtil.dataStreamForFlowSpread);
	    		estimateSpread(GeneralUtil.dataSummaryForFlowSpread);
			}
			String resultFilePath = GeneralUtil.path + "topspread\\CMH\\" + C[0][0].getDataStructureName()
					+ "_M_" +  M / 1024 + "_d_" + d + "_u_" + u + "_m_" + m;
							GeneralUtil.getaverage(resultFilePath);
		}
	...
	public static void estimateSize(String filePath) throws FileNotFoundException {
		...
		Scanner sc = new Scanner(new File(filePath));
		String resultFilePath = GeneralUtil.path + "CM\\size\\" + C[0][0].getDataStructureName()
	...
	public static void encodeSpread(String filePath) throws FileNotFoundException {
		...
		Comparator<Long> com=new Comparator<Long>() {
			   ...
			   public int compare(Long o1,Long o2) {
				   return finalresult[smap.get(o1)]-finalresult[smap.get(o2)];
			   }
		   };
	    fqueue=new PriorityQueue<Long>(com);
	    long[] flowida=new long[60000000];
		long[] elementida=new long[60000000];
		int kkk=0;
		while (sc.hasNextLine()&&kkk<GeneralUtil.totalencode){
			String entry = sc.nextLine();
			String[] strs = entry.split("\\s+");
	    ...
	    while (nnn<kkk) {
			...
			n++;
			for (int i = 0; i < d; i++) {
				int j = (GeneralUtil.intHash(GeneralUtil.FNVHash1(flowida[nnn]) ^ S[i]) % w + w) % w;
				C[i][j].encode(elementida[nnn]);
			}
			/*/	if(!ssset.contains(flowida[nnn])) {
				int estimate=Integer.MAX_VALUE;
				for(int i = 0; i < d; i++) {
					int j = (GeneralUtil.intHash(GeneralUtil.FNVHash1(flowida[nnn]) ^ S[i]) % w + w) % w;
					estimate = Math.min(estimate, C[i][j].getValue());
				...
				if(estimate>=U)
					ssset.add(flowida[nnn]);
			}
		if()
			if(smap.containsKey(flowida[nnn])) {
				fqueue.remove(flowida[nnn]);
				int estimate=Integer.MAX_VALUE;
				for(int i = 0; i < d; i++) {
					int j = (GeneralUtil.intHash(GeneralUtil.FNVHash1(flowida[nnn]) ^ S[i]) % w + w) % w;
					estimate = Math.min(estimate, C[i][j].getValue());
				}
				finalresult[smap.get(flowida[nnn])]=estimate;
				fqueue.offer(flowida[nnn]);
				
			}
			else 
				if(uuu<ssnum) {
					smap.put(flowida[nnn],uuu++);
					int estimate=Integer.MAX_VALUE;
					for(int i = 0; i < d; i++) {
						int j = (GeneralUtil.intHash(GeneralUtil.FNVHash1(flowida[nnn]) ^ S[i]) % w + w) % w;
						estimate = Math.min(estimate, C[i][j].getValue());
					}
					finalresult[uuu-1]=estimate;
					fqueue.offer(flowida[nnn]);
				}
				else {
					int r1=smap.get(fqueue.peek());
					int rr=finalresult[r1];
					int estimate=Integer.MAX_VALUE;
					for(int i = 0; i < d; i++) {
						int j = (GeneralUtil.intHash(GeneralUtil.FNVHash1(flowida[nnn]) ^ S[i]) % w + w) % w;
						estimate = Math.min(estimate, C[i][j].getValue());
					...
					int rrr=estimate;
					if(rrr>rr) {
						smap.remove(fqueue.peek());
						smap.put(flowida[nnn],r1);
						finalresult[r1]=rrr;
						fqueue.poll();
						fqueue.offer(flowida[nnn]);
					}
	...
	public static void estimateSpread(String filepath) throws FileNotFoundException {
		...
		Scanner sc = new Scanner(new File(filepath));
		String resultFilePath = GeneralUtil.path + "topspread\\CMH\\" + C[0][0].getDataStructureName()
				+ "_M_" +  M / 1024 + "_d_" + d + "_u_" + u + "_m_" + m+"_time_"+times;
		PrintWriter pw = new PrintWriter(new File(resultFilePath));
		System.out.println("Result directory: " + resultFilePath); 
		while (sc.hasNextLine()) {
			String entry = sc.nextLine();
		...
		pw.close();
		// obtain estimation accuracy results
		GeneralUtil.analyzetopAccuracycm(resultFilePath);
	}
